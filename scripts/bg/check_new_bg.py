...
def load_existing_bg_assets(file_path: str) -> list:
...
def compare_and_update_bg_assets(existing_assets: list, new_assets: dict) -> list:
    """Compare existing and new background assets and update the existing assets."""
    existing_paths = [bg_obj["path"] for bg_obj in existing_assets]
    new_assets_list = []
    new_assets =json.loads(new_assets)
    for bg in new_assets.keys():
        if bg == "[None]":
            continue
        if new_assets[bg]["res"].split("/")[-1] not in existing_paths:
            my_path = "/".join(new_assets[bg]["res"].split("/")[:-1])
            print("New Background found:", bg)
            d = {"display_name": bg, "path": my_path}
            new_assets_list.append(d)
    return new_assets_list, True if len(new_assets_list) > 0 else False


def save_updated_bg_assets(file_path: str, updated_assets: list):
    """Save the updated background assets to a JSON file."""
    with open(file_path, "w") as f:
        json.dump({"bg": updated_assets}, f, indent=2)


if __name__ == "__main__":
    existing_bg_file = "https://raw.githubusercontent.com/explicitdanger/eggs-db/refs/heads/main/bg_assets.json"  # Path to your existing assets file
    new_bg_url = "https://raw.githubusercontent.com/vega-spica/Personality-Sneak-Peek-Expanded/refs/heads/main/dict/cavebgdict.js"  # URL to fetch new assets

    # Load existing background assets
    existing_assets = load_existing_bg_assets(existing_bg_file)

    # Fetch and parse new background assets
    new_assets = fetch_and_parse_js_object(new_bg_url)

    # Compare and update existing assets
    updated_assets, updated = compare_and_update_bg_assets(existing_assets["bg"], new_assets)

    if updated:
        # Save the updated background assets to a new JSON file
        save_updated_bg_assets("bg/updated_bg_assets.json", updated_assets)
        print("Updated background assets saved to 'updated_bg_assets.json'.")
    else:
