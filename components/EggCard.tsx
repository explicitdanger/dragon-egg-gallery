...
export default function EggCard({ egg, selectedStage }: EggCardProps) {
  return (
    <Link href={`/${formatRouteParam(egg.name)}`} className="block">
      <Card
      ...
      >
        ...
        <div className="absolute top-4 left-0 z-10">
          <div
            className="bg-bistre/90 text-vanilla px-4 py-1.5 rounded-r-full shadow-lg
        ...
        <CardContent className="p-4 bg-gradient-to-b from-chamoisee/5 to-chamoisee/20 backdrop-blur-sm space-y-4">
          <div className="relative px-6">
            <span
              className="absolute left-0 top-0 text-2xl text-bistre/40 font-serif"
              aria-hidden="true"
            >
              &ldquo;
            </span>
            ...
            <span
              className="absolute right-0 bottom-0 text-2xl text-bistre/40 font-serif"
              aria-hidden="true"
            >
              &rdquo;
            </span>
...
function getRarityStyle(rarity: string): string {
  const styles = {
    ...
    legendary:
      "border-yellow-500/50 text-yellow-700 bg-yellow-50 hover:bg-yellow-100",
  };
